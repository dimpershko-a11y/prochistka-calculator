...
function bind(){
  ...
  if($('saveSyncEndpointBtn')) $('saveSyncEndpointBtn').onclick=saveSyncEndpointFromUi;
  if($('checkRemoteConfigBtn')) $('checkRemoteConfigBtn').onclick=()=>checkRemoteConfig(true,false);
  if($('publishConfigBtn')) $('publishConfigBtn').onclick=publishConfigToCloud;
  if($('applyRemoteConfigBtn')) $('applyRemoteConfigBtn').onclick=()=>checkRemoteConfig(true,true);
  ...
  if($('dataModalBtn')) $('dataModalBtn').onclick=()=>{ $('dataModal')?.classList.remove('hidden'); renderSyncStatus(); };
  ...
  $('clearBtn').onclick=()=>{ const currentCleanType=state.form.cleanType; state.form=clone(defaults.form); if(currentCleanType && state.cleaningTypes?.[currentCleanType]) state.form.cleanType=currentCleanType; ensureFormCleanTypeAndCoefs(true); state.extras=state.extras.map(x=>({...x, qty:0})); fillForm(); renderExtras(); recalc(); toast('Форма очищена'); };
  $('resetStorageBtn').onclick=()=>requestEditAccess(()=>{ if(!confirm('Сбросить все сохранённые данные в этом браузере?')) return; localStorage.removeItem(STORAGE_KEY); state=mergeState(clone(defaults)); migrateV43(); migrateV46(); migrateV411(); migrateV412(); migrateV413(); migrateV414(); migrateV415(); migrateV4115(); syncConfigRevision(); fillForm(); renderTariffs(); renderExtras(); recalc(); toast("Все данные сброшены"); });
  $('demoBtn').onclick=()=>{ state.form={...state.form, clientName:'Ирина', objectType:'Квартира', area:68, cleanType:'general', discount:5, forceDiscount:false, clutter:'medium', dirtiness:'medium', travelType:'km15', travelKm:20, managerOnSite:true, ownerRole:'cleaner_manager', profitPercent:25, notes:'Есть кот. Уборка нужна в пятницу после 11:00.'}; state.extras=state.extras.map(x=>({...x, qty:({1:1,9:1,14:4,18:1}[x.id]||0)})); fillForm(); renderExtras(); recalc(); toast('Подставлен пример'); };
  $('showOnlySelected').onchange=e=>{ state.form.showOnlySelected=e.target.checked; saveState(); renderExtras(); };
  if($('forceDiscount')) $('forceDiscount').onchange=e=>{ state.form.forceDiscount=e.target.checked; recalc(); };
  ...
  ['clientName','clientPhone','objectType','cleanDate','address','area','discount','discountAmount','travelKm','maintenanceCrewSize','notes'].forEach(id=>{ const el=$(id); if(!el) return; el.oninput=e=>{ state.form[id]=['area','discount','discountAmount','travelKm','maintenanceCrewSize'].includes(id)?num(e.target.value):e.target.value; recalc(); }; });
  if($('clientPhone')) $('clientPhone').oninput=e=>{ const digits=String(e.target.value||'').replace(/\D/g,'').replace(/^(?:8|7)/,'').slice(0,10); const parts=[digits.slice(0,3),digits.slice(3,6),digits.slice(6,8),digits.slice(8,10)]; e.target.value='+7'+(parts[0]?' ('+parts[0]:'')+(parts[0].length===3?')':'')+(parts[1]?' '+parts[1]:'')+(parts[2]?'-'+parts[2]:'')+(parts[3]?'-'+parts[3]:''); state.form.clientPhone=e.target.value; };
  if($('outsideKad')) $('outsideKad').onchange=e=>{ state.form.outsideKad=e.target.checked; recalc(); };
  if($('managerOnSite')) $('managerOnSite').onchange=e=>{ state.form.managerOnSite=e.target.checked; state.form.ownerRole=e.target.checked?(state.form.ownerRole==='cleaner_manager'?'cleaner_manager':'manager'):'none'; recalc(); };
  ['cleanType','clutter','dirtiness','travelType','ownerRole','discountMode','maintenanceFormat','maintenanceFrequency','maintenanceTerm'].forEach(id=>{ const el=$(id); if(!el) return; el.onchange=e=>{ state.form[id]=e.target.value; if(id==='cleanType'){ ensureFormCleanTypeAndCoefs(true); populateMainSelects(); renderExtras(); } if(id==='discountMode'){ updateDiscountInputs(); } recalc(); }; });
...
fillForm(); renderTariffs(); bind(); renderExtras(); renderSettingsPanel(); setSettingsTab(state.ui.settingsTab); enhanceAccessibility(); recalc(); updateBackupReminder(); attemptIdbRecovery(); renderSyncStatus(); if($('versionBadge')) $('versionBadge').textContent=APP_VERSION; setupAccess();
setTimeout(()=>checkRemoteConfig(false,false),800);
let __lastRemoteConfigCheck=0;
document.addEventListener('visibilitychange',()=>{ if(document.visibilityState!=='visible') return; const now=Date.now(); if(now-__lastRemoteConfigCheck<30000) return; __lastRemoteConfigCheck=now; checkRemoteConfig(false,false); });
