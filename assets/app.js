...
const APP_VERSION = APP_CONFIG.APP_VERSION || 'codex v4.7.2';
...
defaults.form = defaults.form || {clientName:'',objectType:'Квартира',area:0,cleanType:'general',discount:0,discountMode:'percent',discountAmount:0,forceDiscount:false,clutter:'low',dirtiness:'low',travelType:'kad',travelKm:20,ownerRole:'cleaner_manager',profitPercent:25,notes:'',showOnlySelected:false};
...
function renderEconomyWarning(r){
  ...
  if(r.forceDiscount && r.belowDirect){
    el.classList.remove('hidden');
    el.innerHTML = `<strong>Принудительная скидка: итог (${money(r.recommendedPrice)}) ниже прямых затрат (${money(r.directCost)}).</strong><br>
      Защита от убыточной цены отключена вручную, заказ в минус на ${money(r.directCost - r.recommendedPrice)}.`;
  } else if(r.forceDiscount && r.belowFull){
    const mContrib=Math.max(0, r.recommendedPrice - r.directCost);
    el.classList.remove('hidden');
    el.innerHTML = `<strong>Принудительная скидка: итог (${money(r.recommendedPrice)}) ниже полной себестоимости (${money(r.fullCost)}).</strong><br>
      Автоподъём до целевой цены отключён, вклад в накладные ${money(mContrib)}, не хватает ${money(Math.max(0, r.fullCost - r.recommendedPrice))}.`;
  } else if(r.belowDirect){
    el.classList.remove('hidden');
...
function recalc(){
  ...
  $('sumBase').textContent=money(r.baseRaw); $('sumExtras').textContent=money(r.extrasTotal); $('sumTravel').textContent=money(r.travelTotal); $('sumDiscount').textContent=(r.forceDiscount?'− '+money(r.discountValue)+' принуд.':'− '+money(r.discountValue)); $('sumMarket').textContent=money(r.marketPrice);
...
function bind(){
  ...
  $('demoBtn').onclick=()=>{ state.form={...state.form, clientName:'Ирина', objectType:'Квартира', area:68, cleanType:'general', discount:5, forceDiscount:false, clutter:'medium', dirtiness:'medium', travelType:'km15', travelKm:20, ownerRole:'cleaner_manager', profitPercent:25, notes:'Есть кот. Уборка нужна в пятницу после 11:00.'}; state.extras=state.extras.map(x=>({...x, qty:({1:1,9:1,14:4,18:1}[x.id]||0)})); fillForm(); renderExtras(); recalc(); toast('Подставлен пример'); };
